# KPI Matrix With Trend Arrows — Lightdash custom chart type (local copy)
...
Preview locally with the included `viz-fixture.json`: run `lightdash apps preview` (or `npm run dev`) and open the dev URL with `?vizFixture=/viz-fixture.json`. The SDK's dev-only fallback seeds `useVizContext()` from that file so the chart renders without the Lightdash host. Edit the fixture to match your `vizSchema`. It's fake data for layout iteration and never runs in production — the explorer with real data is the source of truth, so upload and verify there too.
